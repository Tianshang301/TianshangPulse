...
| **Client（Health，兼容）** | `BleManager.kt` 使用 `ScanFilter.setServiceUuid(0xFFF0)` 做**硬件级过滤** → 当前**扫不到设备** | 在 Server 修复前**不得**依赖服务 UUID 过滤；改为按 `ScanRecord.deviceName`（或前缀）判定，**连接后以服务发现结果做最终校验** |

**结论**：`0xFFF0` 只能用作「连接后校验」，不能用作「扫描准入」。此项为 **Phase 1 阻塞项，两端同时改**。
...
| 项 | 说明 |
| --- | --- |
| 长度 | **恒 8 字节**（`len < 8` 或 `len > 32` 均被拒） |
| ATT 错误码 | 长度非法 → `0x0D Invalid Attribute Value Length`；CRC 不符 → `0x04 Invalid PDU`（**配置不生效**） |
...
| 项 | 说明 |
| --- | --- |
| 长度 | **恒 2 字节**（以向量 V7/V7b 与 `wire_constraints.0xFFF3_min_length=2` 裁决） |
| ATT 错误码 | `len < 1` → `0x0D`；`idx > 1` → `0x04 Invalid PDU` |
| 🚫 **实现差异（D-A）** | 固件 `on_chr_fff3_write` 目前**只校 `len≥1` 与 `idx≤1`，未校验 CRC、未强制 `len≥2`**；`docs/PROTOCOL.md` §5 也未写 CRC 行。**三处不一致 → 以向量为准**（见 §8） |
...
| --- | --- | --- | --- | --- |
| `O-1` | `PROTOCOL.md` §6 曾写「末尾带 CRC-8」与实现不符，且整批 CRC 恒为 `00` | 若按旧文案实现整批 CRC 会一直校验失败 | **不实现整批 CRC**；按逐条 CRC + `len % 11 == 0` 校验 | Pulse（文档已按实现裁决） |
...
| `O-4` | 广播未设 `0xFFF0` 服务 UUID → **设备发现失败** | `ScanFilter.setServiceUuid` 硬件过滤扫不到 | 见 §3.2，**放宽扫描过滤**（P0） | 两端（Server 先改） |
