...
- 端侧 AI：**7 参数逻辑回归（RR 间期特征）+ 运动门控**，val AUC 0.928（零运行时依赖，见 §4.1）
...
## 4. 输入/输出张量（⚠️ 历史存档：原始波形 LSTM 方案已弃用）

> 本表描述的是**已弃用的 1 秒窗 LSTM** 方案（2026-09-05 起）。
> 当前部署的 LR 分类器输入为 **6 维 RR 间期统计特征**（见 §4.1），不再消费原始波形张量。
> 以下内容仅保留作历史存档与深度模型回归时的参考。
